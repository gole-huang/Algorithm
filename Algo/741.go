...
func firstPath(x, y int, cherry *Cherry) [][][2]int {
	length := len(cherry.Grid)
	var pathX, pathY [][][2]int
	if x == length-1 && y == length-1 {
		return [][][2]int{{{x, y}}}
	} else if x == length-1 {
		pathY = make([][][2]int, 1)
		pathY[0] = make([][2]int, 0)
		for i := y; i < length; i++ {
			if cherry.Grid[x][i][0] {
				return nil
			} else {
				pathY[0] = append(pathY[0], [2]int{x, i})
			}
		}
		return pathY
	} else if y == length-1 {
		pathX = make([][][2]int, 1)
		pathX[0] = make([][2]int, 0)
		for i := x; i < length; i++ {
			if cherry.Grid[i][y][0] {
				return nil
			} else {
				pathX[0] = append(pathX[0], [2]int{i, y})
			}
		}
		return pathX
	} else {
		...
		***本单元格为1或左下方单元格为0*/
		if !cherry.Grid[x+1][y][0] { //添加下方路径
			...
			for i := 0; i < len(pathX); i++ {
				if !cherry.Grid[x+1][y][1] && cherry.Grid[x][y+1][1] && (pathX[i] == nil || pathX[i][len(pathX[i])-1] == [2]int{x + 1, y + 1}) { //当且仅当无左下节点，或者本节点为1或左下节点为0
					pathX[i] = nil
				} else if pathX[i] != nil {
		...
		***本单元格为1且右上方单元格为0*/
		if !cherry.Grid[x][y+1][0] { //添加右方路径
			...
			for i := 0; i < len(pathY); i++ {
				if (cherry.Grid[x+1][y][1] || !cherry.Grid[x][y+1][1]) && (pathY[i] == nil || pathY[i][len(pathY[i])-1] == [2]int{x + 1, y + 1}) { //当且仅当无右上节点，或者本节点为1且右上节点为0
					pathY[i] = nil
				} else if pathY[i] != nil {
...
func secondPath(x, y, target int, cherry *Cherry) int {
	length := len(cherry.Grid)
	if cherry.Grid[x][y][0] || target >= (length)*2-1-x-y {
		return -1
	} else {
		var res int
